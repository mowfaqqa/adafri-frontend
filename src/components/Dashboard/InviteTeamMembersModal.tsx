...
import { Mail, Users, Share2, Clock, Plus, X, Trash2, Copy, Check, Send, RefreshCw, UserPlus, Link, MessageSquare } from 'lucide-react';
...
type InviteType = 'permanent' | 'temporary';
...
export const InviteTeamMembersModal: React.FC<InviteTeamMembersModalProps> = ({ 
  ...
  const [messages, setMessages] = useState<string[]>(['']); // Replace role with message
  const [inviteType, setInviteType] = useState<InviteType>('permanent');
  ...
  const getDefaultMessage = (type: InviteType, duration?: TemporaryDuration): string => {
    if (type === 'permanent') {
      return 'Welcome to our team! You have been invited to join our workspace for permanent collaboration.';
    } else {
      const durationText = getDurationLabel(duration || '1week').toLowerCase();
      return `You have been granted temporary access to our workspace for ${durationText}. Please complete your tasks within the given timeframe.`;
    }
  };
  ...
  const handleSendInvites = async () => {
    try {
      ...
      const invites: InviteRequest[] = validEmails.map((email, index) => ({
        email: email.trim(),
        message: messages[index]?.trim() || getDefaultMessage(inviteType, temporaryDuration),
        inviteType,
  ...
  const handleInviteTypeChange = (type: InviteType) => {
    setInviteType(type);
    // Update all messages with new default based on type
    const newMessages = messages.map(() => getDefaultMessage(type, temporaryDuration));
    setMessages(newMessages);
  };

  const handleDurationChange = (duration: TemporaryDuration) => {
    setTemporaryDuration(duration);
    // Update messages if temporary type is selected
    if (inviteType === 'temporary') {
      const newMessages = messages.map(() => getDefaultMessage('temporary', duration));
      setMessages(newMessages);
    }
  };
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-gray-50 rounded-xl p-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        onClick={() => handleInviteTypeChange('permanent')}
                      ...
                      <button
                        onClick={() => handleInviteTypeChange('temporary')}
                        className={`p-3 rounded-lg border-2 text-left transition-all duration-200 ${
                  ...
                  <div className="space-y-4">
                    <label className="block text-sm font-semibold text-gray-700">Email & Message</label>
                    {emails.map((email, index) => (
                      <div key={index} className="space-y-3 p-4 bg-gray-50 rounded-xl">
                        ...
                        <div className="relative">
                          ...
                          <textarea
                            placeholder={`Enter invitation message for ${inviteType} access...`}
                            className="w-full border-2 border-gray-200 rounded-xl pl-10 pr-4 py-3 focus:border-blue-500 focus:outline-none transition-colors duration-200 min-h-[80px] resize-none"
                            value={messages[index] || getDefaultMessage(inviteType, temporaryDuration)}
...
//                   {/* Role selection - Only show for permanents */}
//                   {inviteType === 'permanent' && (
//                     <div className="bg-gray-50 rounded-xl p-4">
