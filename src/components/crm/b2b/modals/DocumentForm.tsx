...
export const DocumentForm: React.FC<DocumentFormProps> = ({ 
  ...
  const { formatCurrency, settings } = useInvoiceSettings();
  ...
  const formatCurrencyPlaceholder = (amount: number) => {
    return `${settings.selectedCurrency.symbol}${amount.toFixed(2)}`;
  };
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center">
              <span className="text-white font-bold text-sm">{settings.selectedCurrency.symbol}</span>
            </div>
            <div>
              <p className="text-sm font-medium text-gray-900">Selected Currency</p>
              <p className="text-xs text-gray-600">{settings.selectedCurrency.code} - {settings.selectedCurrency.name}</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-sm text-gray-600">All amounts will be displayed in</p>
            <p className="font-medium text-blue-600">{settings.selectedCurrency.code}</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="border border-gray-200 rounded-lg overflow-hidden">
          <div className="bg-gray-50 px-4 py-3 grid grid-cols-12 gap-4 text-sm font-medium text-gray-700">
            ...
            <div className="col-span-2">Unit Price ({settings.selectedCurrency.code})</div>
            <div className="col-span-2">Total ({settings.selectedCurrency.code})</div>
            ...
            <div key={item.id} className="px-4 py-3 grid grid-cols-12 gap-4 border-t border-gray-200">
              ...
              <div className="col-span-2">
                <input 
                  ...
                  min="0" 
                  step="0.01"
                  placeholder={formatCurrencyPlaceholder(0)}
                />
              </div>
              <div className="col-span-2">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gray-50 p-4 rounded-lg space-y-3">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="ml-auto font-medium">-{formatCurrency(document.discountAmount)}</span>
            </div>
            <div className="flex items-center space-x-2">
              ...
              />
              <span className="text-sm text-gray-600">%</span>
              <span className="ml-auto font-medium">{formatCurrency(document.taxAmount)}</span>
            </div>
            <div className="border-t border-gray-200 pt-3">
              <div className="flex justify-between">
                <span className="text-lg font-semibold text-gray-900">Total:</span>
                <span className="text-lg font-bold text-blue-600">{formatCurrency(document.total)}</span>
              </div>
            </div>
          </div>
        </div>
...
// the input field of the invoice form
// "use client";
// import React, { useState } from 'react';
// import { X, Plus, Trash2, Calendar, Building, User, Upload, FileText } from 'lucide-react';
// import type { DocumentData, CompanyInfo, InvoiceItem, DocumentType, Contact } from '@/lib/types/invoice/types';
