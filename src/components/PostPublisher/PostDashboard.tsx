...
const PostDashboard: React.FC = () => {
  ...
  // ADDED: State for prefilled date/time when creating posts from calendar
  const [prefilledDate, setPrefilledDate] = useState<Date | null>(null);
  const [prefilledTime, setPrefilledTime] = useState<string>('09:00');
  
  // ADDED: State for editing posts
  ...
  const handlePublish = (postData: any, publishType: 'now' | 'schedule' | 'draft', scheduledAt?: Date) => {
    ...
    if (editingPost) {
      ...
      const updatedPost: SocialPost = {
        ...
        status: status,
        // Keep existing analytics if it was a published post
        ...(editingPost.analytics && { analytics: editingPost.analytics })
      };
  ...
  // ADDED: Handle edit post from calendar
  const handleEditPostFromCalendar = (post: SocialPost) => {
    console.log('Edit post from calendar:', post);
    setEditingPost(post);
    setPrefilledDate(null); // Clear prefilled date when editing
  ...
  // FIXED: Handle date selection from calendar (this was incomplete before)
  const handleDateSelect = (date: Date) => {
    console.log('Date selected from calendar:', date);
    // Don't automatically open modal when just selecting a date
    // Modal should only open when + button is clicked
  };

  // ADDED: Handle new post creation from calendar + button
  const handleNewPost = (date: Date, time: string) => {
    console.log('New post requested for:', date, 'at time:', time);
    setPrefilledDate(date);
    setPrefilledTime(time);
    setShowComposeModal(true);
  };

  // UPDATED: Handle closing compose modal and reset all states
  ...
  // UPDATED: Handle opening compose modal normally (without prefilled data or editing)
...
//   // UPDATED: Handle closing compose modal and reset prefilled data
//   const handleCloseComposeModal = () => {
//     setShowComposeModal(false);
//     setPrefilledDate(null);
//     setPrefilledTime('09:00');
...
//   // UPDATED: Handle opening compose modal normally (without prefilled date)
//   const handleOpenComposeModal = () => {
//     setPrefilledDate(null);
//     setPrefilledTime('09:00');
//     setShowComposeModal(true);
