...
export function B2CFlowManager({ onReconfigure }: B2CFlowManagerProps) {
  ...
  return (
    <div className="min-h-screen bg-white">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
            ...
            <div className="bg-gray-200 rounded-xl p-1 inline-flex">
              {tabs.map((tab) => (
                ...
                >
                  {tab.label}
                  {/* Show deal count badge for Deal tab */}
                  {tab.id === 'Deal' && contacts.filter(c => c.status !== 'Supplier').length > 0 && (
                    <span className="ml-2 bg-orange-100 text-orange-600 text-xs px-2 py-1 rounded-full">
                      {contacts.filter(c => c.status !== 'Supplier').length}
                    </span>
                  )}
                </button>
              ))}
      ...
      {/* Debug/Reconfigure button - Adjusted z-index to work with deals view */}
      <button
      ...
      {/* Toast container for deal movements - Ensure it appears above other elements */}
      <div id="toast-container" className="fixed top-0 left-0 w-full h-full pointer-events-none z-50" />
    </div>
  );
}
