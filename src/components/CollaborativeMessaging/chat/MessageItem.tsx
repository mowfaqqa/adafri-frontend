...
const MessageItem: React.FC<MessageItemProps> = ({ message, isOwnMessage }) => {
  ...
  // Safe date parsing and formatting - ENHANCED VERSION
  const formatTime = (date: Date | string | null | undefined) => {
    try {
      ...
      if (date instanceof Date) {
        ...
        if (date.trim() === '') {
          console.warn('Date is empty string');
          return 'No date';
        }
        parsedDate = new Date(date);
      } else {
        // Fallback for invalid date types
        console.warn('Invalid date type provided to formatTime:', typeof date, date);
        return 'Invalid date';
      }
      ...
      <div className="flex space-x-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center">
            <span className="font-semibold">{message.sender?.fullName || 'Unknown User'}</span>
            <span className="ml-2 text-xs text-gray-500">
              {(() => {
                // Debug logging for date issues
                console.log('Message createdAt value:', message.createdAt, 'Type:', typeof message.createdAt);
