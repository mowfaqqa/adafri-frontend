...
import { useInvoiceSettings , POPULAR_CURRENCIES, OTHER_CURRENCIES, type Currency } from '@/lib/context/invoices/InvoiceSettingsProvider';
...
export function TemplateCurrencySelector({ 
  ...
  const isSelectedCurrency = (currency: Currency) => {
    return settings.selectedCurrency.code === currency.code;
  };

  const isSelectedCurrencyInPopular = () => {
    return POPULAR_CURRENCIES.some(currency => currency.code === settings.selectedCurrency.code);
  };
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl mx-auto">
            ...
            {POPULAR_CURRENCIES.map((currency) => {
              const isSelected = isSelectedCurrency(currency);
            ...
            <div className="relative">
              <button
                ...
                className={`relative p-4 rounded-xl border-2 transition-all duration-300 text-left group hover:shadow-md w-full ${
                  !isSelectedCurrencyInPopular() && settings.selectedCurrency.code !== 'USD'
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="flex items-center space-x-2">
                          <span className="font-medium text-gray-900">More Currencies</span>
                        </div>
                        <p className="text-sm text-gray-600">
                          {!isSelectedCurrencyInPopular() && settings.selectedCurrency.code !== 'USD'
                            ? `${settings.selectedCurrency.code} - ${settings.selectedCurrency.name}`
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              {currency.region && (
                                <p className="text-xs text-gray-500">{currency.region}</p>
                              )}
                            </div>
                          </div>
                          {isSelected && (
